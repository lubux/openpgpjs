...
module.exports = () => describe('ECDH key exchange @lightweight', function () {
  ...
  it('Successful exchange x25519', async function () {
    const { ecdhMontgomery } = elliptic_curves;
  ...
  it('Successful exchange x448', async function () {
    ...
    const { ephemeralPublicKey, wrappedKey } = await ecdhMontgomery.encrypt(openpgp.enums.publicKey.x448, data, K_B);
    expect(await ecdhMontgomery.decrypt(openpgp.enums.publicKey.x448, ephemeralPublicKey, wrappedKey, K_B, b)).to.deep.equal(data);
  });
